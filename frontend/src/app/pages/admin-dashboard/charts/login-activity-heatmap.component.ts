...
@Component({
  selector: 'app-login-activity-heatmap',
  standalone: true,
  imports: [CommonModule],
  template: `<canvas #chartCanvas></canvas>`
})
export class LoginActivityHeatmapComponent implements AfterViewInit {

  private API = 'http://localhost:5004/api/admin/login-activity';
  private chart!: Chart;
  ...
  private loadChart() {
    ...
    this.http.get<any[]>(this.API, this.auth()).subscribe(data => {
      ...
      this.chart = new Chart(this.canvas.nativeElement, {
        type: 'bar',
        data: {
          labels: data.map(d => `${d.day} ${d.hour}:00`),
          datasets: [{
            label: 'Logins',
